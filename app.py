...
def sendfile():
    if request.method == 'POST':
        ...
        content_target_image = Image.open(BytesIO(base64.b64decode(data['content_target_image'])))
        content_target_image = convert_to_rgb(content_target_image)
        content_source_image = data['content_source_image']
        
        # 이미지 이름 중복성 검사
        timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
        content_target_name = f"{timestamp}_{content_target_name}"
        data['content_target_name'] = content_target_name
        ...
        resizing_len = 1000 # 최대 이미지 크기
        longest_len = max(width, height)
        
        if longest_len > resizing_len:
            if width < height:
                decay_rate = round(resizing_len/height, 2)
            else:
                decay_rate = round(resizing_len/width, 2)
        ...
        if content_source_image is not None:
            ...
            data['content_source_image'] = convert_to_rgb(content_source_image.resize((width, height)))
        data['style_image'] = convert_to_rgb(style_image.resize((width, height)))
        ...
        print(path)
        # 이미지 파일 이름 변경
        print(content_target_name)
        
        # 이미지 저장
        image.save(path + "/" + content_target_name)
...
def gen_image():
    if request.method == "POST":
        ...
        image = gen_model.processing(prompt=prompt)
