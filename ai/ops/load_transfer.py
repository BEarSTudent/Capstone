...
class Transfer:
    ...
    def content_image_check_size(self, content_image:Image):
        self.image_width, self.image_height = content_image.size
        width, height = self.image_width//self.image_decay_rate, self.image_height//self.image_decay_rate
        
        self.loader = transforms.Compose([
                transforms.Resize((height, width)),  # 가져온 이미지 크기 조정
                transforms.ToTensor()])  # 텐서로 변환
        
    def image_loader(self, image: Image):
...
if __name__ == "__main__":
    style_image = Image.open("./ai/data/style/dall_e.jpeg")
    content_image = Image.open("./ai/data/content/image_2.jpeg")
    model = Transfer()
    image = model.run_style_transfer(content_img=content_image, style_img=style_image)
    image_save(image, "result03")